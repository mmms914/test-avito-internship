...
## docker-compose
Возможно, из-за того что на локальном ПК и у проверяющей системы разные версии docker-compose, почему-то не хотели подставляться переменные окружения, хотя локально все работало - вписал в docker-compose.yaml захардкоженные значения, а не чере файл